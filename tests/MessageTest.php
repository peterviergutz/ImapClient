...
{
    /**
     * @var \sgoranov\ImapClient\Mailbox
     */
    protected $mailbox;
    ...
    public function testEmailAddress()
    {
        ...
        $this->assertInstanceOf('\sgoranov\ImapClient\Message\EmailAddress', $from);
        $this->assertEquals('no_host', $from->getMailbox());

        $cc = $message->getCc();
        $this->assertCount(2, $cc);
        $this->assertInstanceOf('\sgoranov\ImapClient\Message\EmailAddress', $cc[0]);
        $this->assertEquals('This one is right', $cc[0]->getName());
        $this->assertEquals('ding@dong.com', $cc[0]->getAddress());
        
        $this->assertInstanceOf('\sgoranov\ImapClient\Message\EmailAddress', $cc[1]);
